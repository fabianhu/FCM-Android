...
public class kmlBuilder {
    ...
    private String getExtendedData(ArrayList<GpsFrame> track) {
        LineBuilder builder = new LineBuilder();
        builder.add("<ExtendedData>");
        builder.add(kmlBuilder.version);
        for (GpsFrame loc : track) {
            builder.add("<" + GpsFrame.Frame + ">");
            builder.add("<" + GpsFrame.Position + " lon=\"" + loc.getLongitude() + "\" lat=\"" + loc.getLatitude() + "\" height = \"" +
                    +loc.getAltitude() + "\"/>");
            builder.add("<" + GpsFrame.Speed + " vx=\"" + loc.getxSpeed() + "\" vy=\"" + loc.getySpeed() + "\" vz = \"" +
                    +loc.getzSpeed() + "\"/>");
            builder.add("<" + GpsFrame.DistanceToHome + " dx=\"" + loc.getxDist() + "\" dy=\"" + loc.getyDist() + "\" dz = \"" +
                    +loc.getzDist() + "\"/>");
            builder.add("</" + GpsFrame.Frame + ">");
        }
        builder.add("</ExtendedData>");
